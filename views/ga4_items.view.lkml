...
# Measures
  ...
  measure: total_product_views {
    type: count
    filters: [event_name: "view_item"]
  }

  measure: total_product_add_to_carts {
    type: count
    filters: [event_name: "add_to_cart"]
  }

  measure: total_product_purchases {
    type: count
  ...
  measure: product_conversion_rate {
    type: number
    sql: SAFE_DIVIDE(${total_product_purchases} , ${total_product_views}) ;;
    value_format: "0.00%"
  }

  measure: product_ATC_rate {
    type: number
    sql: SAFE_DIVIDE(${total_product_add_to_carts} , ${total_product_views}) ;;
    value_format: "0.00%"
  }
...
# include: "ga4.model.lkml"

view: product_performance {
  derived_table: {
    explore_source: ga4_events {
      column: total_product_views { field: ga4_items.total_product_views }
      column: total_product_add_to_carts { field: ga4_items.total_product_add_to_carts }
      column: product_ATC_rate { field: ga4_items.product_ATC_rate }
      column: total_product_purchases { field: ga4_items.total_product_purchases }
      column: product_conversion_rate { field: ga4_items.product_conversion_rate }
      column: item_name { field: ga4_items.item_name }
    }
  }
  dimension: total_product_views {
    description: ""
    type: number
  }
  dimension: total_product_add_to_carts {
    description: ""
    type: number
  }
  dimension: product_ATC_rate {
    description: ""
    value_format: "0.00%"
    type: number
  }
  dimension: total_product_purchases {
    description: ""
    type: number
  }
  dimension: product_conversion_rate {
    description: ""
    value_format: "0.00%"
    type: number
  }
